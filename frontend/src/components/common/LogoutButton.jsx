...
const LogoutButton = ({ onLogoutSuccess }) => {
  ...
  const handleLogout = async () => {
    setIsLoading(true);
    try {
      // ✅ Call AuthContext logout (handles everything: backend API + state)
      await logout();

      // ✅ Callback if provided (closes navbar dropdown, mobile menu)
      if (onLogoutSuccess) {
        onLogoutSuccess();
      }

      // ✅ AuthContext already shows toast and redirects to home
    } catch (error) {
      console.error('Logout error:', error);
      toast.error(error?.message || 'Logout failed');
      setIsLoading(false);
