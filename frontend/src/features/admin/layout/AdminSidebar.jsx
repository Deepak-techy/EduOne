...
const AdminSidebar = ({ open, setOpen }) => {
  ...
  return (
    <aside
      className="fixed top-0 left-0 h-screen z-50 flex flex-col transition-all duration-300 ease-in-out bg-white border-r border-gray-200"
    ...
    >
      ...
      <div className="flex items-center h-16 px-4 border-b border-gray-100 flex-shrink-0">
        ...
        {open && (
          <div className="ml-3 overflow-hidden">
            <h1 className="text-sm font-bold text-gray-900 tracking-wide">EduOne</h1>
            <p className="text-[10px] text-blue-500 font-semibold">Admin Panel</p>
          </div>
        )}
        <button
          onClick={() => setOpen(o => !o)}
          className="ml-auto p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors cursor-pointer"
        >
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3 px-2" style={{ scrollbarWidth: 'none' }}>
        {MENU.map((item, idx) => {
          if (item.section) {
            return open ? (
              <div key={idx} className="px-3 pt-4 pb-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                {item.section}
              </div>
            ) : (
              <div key={idx} className="mx-3 my-2 border-t border-gray-100" />
            );
          }
          ...
          return (
            ...
            >
              <Icon className={`w-[18px] h-[18px] flex-shrink-0 ${isActive ? 'text-blue-600' : ''}`} strokeWidth={isActive ? 2.5 : 2} />
        ...
        {open ? (
          <div className="flex items-center gap-3 p-2 rounded-xl bg-gray-50">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900 truncate">{admin?.fullName || admin?.userName || 'Admin'}</p>
              <p className="text-[11px] text-gray-400 truncate">{admin?.email || ''}</p>
            </div>
            <button onClick={logout} title="Logout" className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors cursor-pointer">
              <LogOut className="w-4 h-4" />
            </button>
          </div>
        ) : (
          <button onClick={logout} title="Logout" className="w-full flex justify-center p-2.5 rounded-xl hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors cursor-pointer">
