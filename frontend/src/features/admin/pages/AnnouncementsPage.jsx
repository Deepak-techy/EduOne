...
const AnnouncementsPage = () => {
  ...
  const audienceBadge = {
    ...
    Students: 'bg-emerald-50 text-emerald-600 border-emerald-200',
    Teachers: 'bg-amber-50 text-amber-600 border-amber-200',
  };

  if (loading) return (
    <div className="space-y-4">
      {[1,2,3].map(i => (
        <div key={i} className="bg-white shadow-sm rounded-2xl p-6 border border-gray-100 animate-pulse">
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center py-24 gap-4">
      ...
      <button onClick={fetchData} className="px-5 py-2.5 rounded-xl bg-blue-50 text-blue-600 text-sm font-bold cursor-pointer flex items-center gap-2 hover:bg-blue-100 transition">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total', value: totalCount, icon: Megaphone, gradient: 'from-[#2196F3] to-[#00BCD4]', bg: 'bg-blue-50' },
          { label: 'Pinned', value: pinnedCount, icon: Pin, gradient: 'from-amber-500 to-orange-500', bg: 'bg-amber-50' },
          { label: 'Active', value: activeCount, icon: Bell, gradient: 'from-emerald-500 to-teal-500', bg: 'bg-emerald-50' },
        ].map(stat => (
          <div key={stat.label} className="bg-white shadow-sm rounded-2xl p-5 border border-gray-100 flex items-center gap-4 hover:border-gray-200 transition-all">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          {['all', 'pinned', 'inactive'].map(tab => (
            <button key={tab} onClick={() => setFilterTab(tab)}
              className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
                filterTab === tab ? 'bg-blue-100 text-blue-600 border border-blue-200' : 'bg-gray-50 text-gray-500 border border-gray-100 hover:bg-gray-100'
              }`}>
              {tab}
            </button>
          ))}
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button onClick={openCreate}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#2196F3] to-[#00BCD4] text-gray-900 text-sm font-bold shadow-lg shadow-blue-500/15 hover:shadow-blue-500/20 transition-all cursor-pointer whitespace-nowrap">
      ...
      {filtered.length === 0 ? (
        <div className="bg-white shadow-sm rounded-2xl border border-gray-100 p-16 text-center">
          ...
          <p className="text-sm text-gray-400 mb-6">Create your first announcement to get started.</p>
          <button onClick={openCreate} className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#2196F3] to-[#00BCD4] text-gray-900 text-sm font-bold cursor-pointer">
            Create Announcement
          </button>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map(a => {
            ...
            return (
              <div key={aid} className={`group bg-white shadow-sm rounded-2xl border transition-all duration-300 hover:border-gray-300 hover:shadow-lg hover:shadow-gray-200/50 ${
                !isActive ? 'opacity-60 border-gray-200' : a.isPinned ? 'border-blue-200 shadow-blue-100' : 'border-gray-100'
              }`}>
                <div className="p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className={`flex-shrink-0 w-11 h-11 rounded-xl bg-gradient-to-br ${gradientClass} flex items-center justify-center shadow-md`}>
                      {a.isPinned ? <Pin className="w-5 h-5 text-gray-900" /> : <Megaphone className="w-5 h-5 text-gray-900" />}
                    </div>

                    {/* Content */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-sm font-bold text-gray-900 truncate">{a.title || 'Untitled'}</h3>
                        {a.isPinned && <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-blue-100 text-blue-600 border border-blue-200 uppercase">Pinned</span>}
                        {!isActive && <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-red-100 text-red-500 border border-red-200 uppercase">Inactive</span>}
                      </div>
                      <p className="text-sm text-gray-500 line-clamp-2 leading-relaxed">{a.content || ''}</p>
                      <div className="flex items-center flex-wrap gap-3 mt-3">
                        <span className="text-[11px] text-gray-400 flex items-center gap-1"><Clock className="w-3 h-3" /> {date}</span>
                        ...
                        {a.createdBy && <span className="text-[11px] text-gray-400">by {a.createdBy.fullName || a.createdBy.userName || '—'}</span>}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                      <button onClick={() => handleToggleActive(a)} title={isActive ? 'Deactivate' : 'Activate'}
                        className="p-2 rounded-lg hover:bg-gray-50 text-gray-400 hover:text-gray-900 cursor-pointer transition">
                        {isActive ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                      </button>
                      <button onClick={() => handlePin(aid)} title={a.isPinned ? 'Unpin' : 'Pin to top'}
                        className={`p-2 rounded-lg hover:bg-gray-50 cursor-pointer transition flex items-center gap-1 ${a.isPinned ? 'text-blue-600 bg-blue-50' : 'text-gray-400 hover:text-blue-600'}`}>
                      ...
                      <button onClick={() => setConfirmDel(aid)} disabled={deleting === aid} title="Delete"
                        className="p-2 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 cursor-pointer transition disabled:opacity-50">
        ...
        <form onSubmit={handleSave} className="space-y-5">
          <div>
            ...
            <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Announcement title…"
              className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-300 transition" />
          </div>
          <div>
            ...
            <textarea rows={4} value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} placeholder="Write your announcement…"
              className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-300 transition resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.targetAudience} onChange={e => setForm(f => ({ ...f, targetAudience: e.target.value }))}
                className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-300 transition">
            ...
            <div className="flex flex-col justify-end gap-3">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" checked={form.isPinned} onChange={e => setForm(f => ({ ...f, isPinned: e.target.checked }))}
                  className="w-4 h-4 rounded border-gray-300 bg-gray-50 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                <span className="text-xs font-bold text-gray-700"><Pin className="w-3 h-3 inline mr-1" />Pin to top</span>
              </label>
            </div>
          </div>
          ...
          {!editItem && (
            <div className="flex items-center gap-3 p-4 rounded-xl bg-blue-50 border border-blue-200">
              ...
              <label htmlFor="notifyUsers" className="cursor-pointer select-none">
                <span className="text-xs font-bold text-blue-600 flex items-center gap-1.5"><Send className="w-3 h-3" /> Send notification to all users</span>
                <p className="text-[10px] text-gray-400 mt-0.5">Users will see this in their notification center</p>
              </label>
            </div>
          )}
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setShowModal(false)}
              className="px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-gray-50 cursor-pointer transition">
              Cancel
            </button>
            <button type="submit" disabled={saving}
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#2196F3] to-[#00BCD4] text-gray-900 text-sm font-bold shadow-lg hover:shadow-xl transition-all cursor-pointer disabled:opacity-60 flex items-center gap-2">
