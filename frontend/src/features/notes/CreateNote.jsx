...
const convertMarkdownToHTML = (markdown) => {
  return markdown
    ...
    .replace(/``````/g, (match, lang, code) => {
      return `<pre style="background-color: #f8fafc; border: 2px solid #e2e8f0; padding: 12px; border-radius: 8px; overflow-x: auto; margin: 8px 0;"><code style="font-family: 'Courier New', monospace; font-size: 13px; color: #1e293b; display: block;">${code.trim().replace(/</g, '&lt;').replace(/>/g, '&gt;')}</code></pre>`;
    })
    // Inline code (`code`)
    .replace(/`([^`]+)`/g, '<code style="background-color: #dbeafe; padding: 2px 6px; border-radius: 4px; font-family: monospace; color: #1e40af; font-size: 0.9em;">$1</code>')
    // Bold (**text**)
    .replace(/\*\*([^\*]+)\*\*/g, '<strong>$1</strong>')
    // Italic (*text*)
    .replace(/\*([^\*]+)\*/g, '<em>$1</em>')
    // Headings
    .replace(/^### (.+)$/gm, '<h3 style="font-size: 1.1em; font-weight: 600; margin-top: 12px; margin-bottom: 6px;">$1</h3>')
    .replace(/^## (.+)$/gm, '<h2 style="font-size: 1.25em; font-weight: 700; margin-top: 16px; margin-bottom: 8px;">$1</h2>')
    .replace(/^# (.+)$/gm, '<h1 style="font-size: 1.5em; font-weight: 700; margin-top: 20px; margin-bottom: 10px;">$1</h1>')
    // Bullet lists (- item or * item)
    .replace(/^\s*[-*]\s+(.+)$/gm, '<li style="margin-left: 20px;">$1</li>')
    // Numbered lists (1. item)
    .replace(/^\s*\d+\.\s+(.+)$/gm, '<li style="margin-left: 20px;">$1</li>')
    // Line breaks
    .replace(/\n\n/g, '<br><br>')
    .replace(/\n/g, '<br>');
};
...
const CreateNote = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      let uploadNoteId = currentNoteId;
