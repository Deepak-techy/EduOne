                  ...
                  {chatHistory.map((chat, index) => (
                    <div key={index} className={`flex ${chat.type === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                      <div className={`max-w-[80%] px-5 py-3 rounded-2xl shadow-lg ${
                        ...
                        {/* Conditional rendering: user/system/error messages as plain text, AI messages with markdown */}
                        {chat.type === 'user' || chat.type === 'system' || chat.type === 'error' ? (
                          <p className="text-sm leading-relaxed">{chat.message}</p>
                        ) : (
                        ...
                        {chat.source && chat.type === 'ai' && (
                          <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                            <p className="text-xs opacity-75">📄 {chat.source}</p>
                          </div>
                        )}
                      </div>
                    </div>
