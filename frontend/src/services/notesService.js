...
generateTags: async (noteId, subject, content) => {
  ...
  // 10. Get tag suggestions - ✅ MATCHES BACKEND
getTagSuggestions: async (query = '') => {
  try {
    const response = await api.get(`/tag-suggestions?query=${query}`);
    return response.data;
  } catch (error) {
    console.error('Get tag suggestions error:', error);
    throw error.response?.data || { message: 'Failed to fetch tag suggestions' };
...
// ✅ Delete document/PDF from upload pdf in note create page 

deleteDocument: async (noteId) => {
  try {
    const response = await api.delete(`/${noteId}/delete-document`);
    return response.data;
  } catch (error) {
...
getSubjects: async () => {
  try {
    ...
    // Response structure: { statusCode: 200, data: ["DSA", "Machine learning"], message: "...", success: true }
    return response.data;
  } catch (error) {
